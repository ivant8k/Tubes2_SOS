...
func Scraper() {
	...
	// Manually add special element "Time" with Tier 0
	...
	// Save to combination.json
	data, err := json.MarshalIndent(allElements, "", "  ")
	if err != nil {
		panic(err)
	}
	err = os.WriteFile("combination.json", data, 0644)
	if err != nil {
		panic(err)
	}

	fmt.Println("✅ Data berhasil disimpan di output.json")
}
